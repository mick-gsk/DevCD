...
**Tests passing is a floor, not a ceiling.**
Passing all tests only proves the code doesn't visibly break existing expectations —
it does NOT prove the work meets the user's actual request.
...
This rule applies even when 100% of tests pass. Even when CI is fully green. Even
when the agent "feels confident".
...
For DevCD: the standard verification command is `make check` (lint + typecheck + pytest).
...
**Tests:**
```
✅ [Run make check] → [See: N passed] → "All tests pass"
❌ "Should pass now" / "Looks correct"
```

**Requirements:**
```
✅ Re-read spec/issue → Create checklist → Verify each → Report gaps or completion
...
Run `make check`. Read the output. THEN claim the result.
