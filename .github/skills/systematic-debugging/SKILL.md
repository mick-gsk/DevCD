...
- Test failures in `make check`
- Bugs in FastAPI routes or service logic
- Unexpected behavior in policy/memory/state slices
- Build failures
- Integration issues between slices
...
4. **Gather Evidence in Multi-Component Systems**
   - For EACH component boundary (slice → kernel, service → API): log what enters and exits
...
3. **Verify Fix** — `make check` passes? Issue actually resolved?
4. **If Fix Doesn't Work** — STOP. Count: How many fixes have you tried?
   - If < 3: Return to Phase 1, re-analyze with new information
   - **If ≥ 3: STOP and question the architecture** — discuss with your human partner before attempting more fixes
...
| **4. Implementation** | Create test, fix, verify | Bug resolved, `make check` passes |
