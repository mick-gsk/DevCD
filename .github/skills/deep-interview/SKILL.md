...
```text
"I'll run a structured interview to clarify your requirements. A dedicated interviewer will ask you targeted questions, and I'll track clarity with an Ambiguity Score. When your requirements are clear enough (score ≤ 0.2), we'll wrap up."
```
...
Agent(
  ...
  prompt="Find: existing patterns, architecture, and code related to [topic]. Report relevant files as file:line format. Keep findings concise."
)
```
...
- Receive the user's answer to the previous question.
- Increment round counter: `round += 1`.
- Launch interviewer agent with the user's answer and selected probe direction:
...
Round: [N] of 10
Conversation so far: [summary of key points from previous rounds]
User's latest answer: [user's answer]
Ask the next question.
Target the area with least clarity.
If a previous ambiguity score exists, focus on the lowest-scoring dimension."
)
```

- Present the interviewer's question to the user.

### 2.3 Ambiguity Scoring (after round ≥ 3)

After every round from round 3 onward, compute the Ambiguity Score.

Scoring method, via LLM self-assessment:
...
```text
📊 Ambiguity: [score] (Goal: [g], Constraints: [c], Criteria: [s]) [progress bar ████████░░ ]
```

Scoring rules:

- Be conservative — score low when uncertain rather than optimistic.
- A dimension scores above 0.8 only when the user has given specific, concrete answers.
- `I don't know` responses mean that dimension stays low, but should be captured as an Open Question.
- Vague answers like `it should be fast` keep Goal Clarity low until quantified.
...
After every 3 rounds, offer a direction change:
...
Use the insights.md template below. After saving, re-present the Next Steps question without `Save insights`.

#### Hand off to /specify

- Save insights to `.hoyeon/deep-interview/[topic-slug]/insights.md` if not already saved.
- Generate the handoff command:

```text
"Ready to plan. Run: /specify --context .hoyeon/deep-interview/[topic-slug]/insights.md \"[1-line topic summary]\""
```
...
Say: `"Good interview. The insights are in your conversation history if you need them later."` Stop.
...
4. No `AskUserQuestion` for probes — Socratic questions come from the interviewer agent. Reserve `AskUserQuestion` for meta-decisions like direction selection and next steps.
...
A round is one cycle: interviewer question → user answer. The following do not count as rounds:

- `AskUserQuestion` meta-decisions such as direction selection and next steps
...
4. Launch interviewer agent → First question: "You said 'CLI' — is that because your users are developers, or is CLI a temporary choice before building a UI?"
...
7. Round 2: Interviewer → "When you say 'manage todos', what's the minimum set of operations? Create-Read-Update-Delete, or something beyond CRUD?"
8. User: "CRUD + priorities, due dates, tags"
9. Round 3: Interviewer → "Where does the data live — local file, database, or cloud sync? Each forces a very different architecture."
   📊 Ambiguity: 0.55 (Goal: 0.6, Constraints: 0.4, Criteria: 0.3)
10. User: "Local JSON file, keep it simple"
11. Round 4: Interviewer → "What does 'done' look like for v1? If you ship tomorrow, what's the one thing it absolutely must do?"
   📊 Ambiguity: 0.35 (Goal: 0.75, Constraints: 0.7, Criteria: 0.5)
12. User: "Add a todo, list them, mark as done"
13. Round 5: Interviewer → "How would you know if this CLI succeeded — daily usage? Replaced another tool? Specific task completion rate?"
   📊 Ambiguity: 0.18 ✅ (Goal: 0.85, Constraints: 0.8, Criteria: 0.85)
14. "Requirements are clear enough. Ready to synthesize?" → User selects "Wrap up"
...
- [ ] Ambiguity Score computed at least once, after round 3 unless in Just discuss mode
...
- [ ] insights.md saved, if the user chose to save
- [ ] /specify handoff command generated, if the user chose handoff